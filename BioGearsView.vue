<template>
	<v-tabs large v-model="tab">
		<v-tab value="home">
			<v-icon start> mdi-home </v-icon> About this page
		</v-tab>
		<v-tab value="option-1">
			<v-icon start> mdi-account </v-icon> Patient Information
		</v-tab>
		<v-tab value="option-2">
			<v-icon start> mdi-access-point </v-icon> Patient Vitals
		</v-tab>
		<v-tab value="option-3">
			<v-icon start> mdi-access-point </v-icon> Environment Properties
		</v-tab>
		<v-tab value="option-4">
			<v-icon start> mdi-list-status </v-icon> Configure Patient Scenario
		</v-tab>
	</v-tabs>
	<v-toolbar color="#3c2d70" style="color: white">
		<v-toolbar-title class="text-center" :style="{ fontSize: '38px', fontWeight: 'bold', transform: 'translateX(-10px)' }">BioGears Scenario Creation Tool</v-toolbar-title>
	</v-toolbar>
					...
					<v-window-item value="home">
						<v-card color="white" flat>
							...
							<v-card flat>
								<v-card-text>
									<v-expansion-panels>
										<v-expansion-panel style='font-size: 25px' title="BioGears Guide">
											<v-expansion-panel-text>
												<p style="font-size: 14px"> After generating the Custom Patient and Scenario files using the BioGears Scenario Creation Tool tab, users must run BioGears with the files in order to generate a MoHSES-compatible state file. The following are instructions on how to run BioGears: </p>
												<v-list>
													<v-list-item style="font-size: 14px">
														<v-list-item-title style="font-size: 18px" class='font-weight-bold'> 1. Build BioGears </v-list-item-title> Create a folder in your Desktop titled 'biogears'. In the command line, navigate to this folder. Clone the BioGears <a href="https://github.com/BioGearsEngine/core">codebase</a> to your local computer by running:
														<pre>git clone https://github.com/BioGearsEngine/core.git </pre> in command line. Follow the build instructions from the <a href="https://github.com/BioGearsEngine/core/wiki/Build-Instructions">Github Wiki</a> for your operating system (also described below). <v-expansion-panels>
															<v-expansion-panel style='font-size: 25px' title="For MacOS (M1/M2 chip)">
																<v-expansion-panel-text style="font-size: 14px"> Install Homebrew by running:
																	<pre>/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)" </pre> in command line. Add Homebrew to path using:
																	<pre>echo 'eval "$(/opt/homebrew/bin/brew shellenv)"' >> ~/.zprofile </pre>
																	<pre>eval "$(/opt/homebrew/bin/brew shellenv)"  </pre> Install dependencies using:
...
brew install xsd
brew install cmake
                            </pre> Assuming the BioGears repository was cloned in the biogears folder on Desktop, run the following commands to build BioGears:
...
ninja bg-cli
															...
															<v-expansion-panel title="For Windows">
																<v-expansion-panel-text style="font-size: 14px"> To build BioGears on a Windows Machine, you will need to download the <a href="https://docs.chocolatey.org/en-us/choco/setup"> Chocolatey Package Manager</a>. Then, install dependencies using:
...
cmake -G -DCMAKE_INSTALL_PREFIX=C:~/Downloads/BioGears/external/ .. </pre>
													...
													<v-list-item style="font-size: 14px">
														<v-list-item-title style="font-size: 18px" class='font-weight-bold'>2. Move downloaded files to proper directories</v-list-item-title> After clicking Submit in the BioGears Scenario Creation Tool tab, two files should be downloaded - CustomScenario.xml and CustomPatient.xml. Within the BioGears directory folder, move CustomScenario.xml to the <i>core/build/runtime/Scenarios/Patient</i> folder. Also within the BioGears directory folder, move CustomPatient.xml to the <i>core/build/runtime/patients</i> folder. If a Custom Environment was created, move the CustomEnvironment.xml file to <i>core/build/runtime/environments</i>.
													</v-list-item>
													<v-list-item style="font-size: 14px">
														<v-list-item-title style="font-size: 18px" class='font-weight-bold'>3. Run BioGears executable in command line</v-list-item-title> Open command line and within the BioGears directory, navigate to <i>core/build/runtime</i>. Run the BioGears executable by running this command: <br>
														<pre>../outputs/Release/bin/bg-cli SCENARIO ./Scenarios/Patient/CustomScenario.xml </pre> This may take a couple minutes to run. After BioGears has finished running, within the <i>Scenarios/Patient folder</i>, there should be CustomScenario.log and CustomScenario.csv files. The csv file can be inputted into the BioGears Plotting Tool Tab to visualize the simulated physiological variables over time. <br> Most importantly, under the <i>core/build/runtime/states</i> folder, a new file called CustomBioGearsState.xml will be created, which is the MoHSES-compatible state file that can be used to generate the MoHSES Scenario file.
													</v-list-item>
												</v-list>
											</v-expansion-panel-text>
										</v-expansion-panel>
					...
					<v-window-item value="option-2">
						<v-card flat>
							<v-card-text>
								...
								<v-slider v-model="patient_vitals['DiastolicArterialPressureBaseline']" label="Diastolic Arterial Pressure" max="80" min="60" step="0.5">
									<template v-slot:append>
										<v-text-field v-model="patient_vitals['DiastolicArterialPressureBaseline']" hide-details single-line density="compact" type="number" suffix="mmHg" style="width: 180px"></v-text-field>
									</template>
								</v-slider>
								<v-slider v-model="patient_vitals['SystolicArterialPressureBaseline']" label="Systolic Arterial Pressure" max="120" min="90" step="0.5">
									<template v-slot:append>
										<v-text-field v-model="patient_vitals['SystolicArterialPressureBaseline']" hide-details single-line density="compact" type="number" suffix="mmHg" style="width: 180px"></v-text-field>
									</template>
								</v-slider>
						...
						<v-card flat>
							<v-card-text>
								<p style="font-size: 20px">
									<b>Optional:</b> Please enter environment properties:
								</p>
								<br>
								...
								<v-row>
									...
									<v-col>
										<div v-for="(item, index) in action" :key='index'>
											...
											<v-slider v-if="item.type==='Hemorrhage'" v-model="item.hemrate" label="Initial Rate" suffix="mL/min" :max="1000" :min="0" :step="1">
												<template v-slot:append>
													<v-text-field v-model="item.hemrate" hide-details single-line suffix="mL/min" density="compact" style="width: 180px"></v-text-field>
												</template>
											</v-slider>
											<v-select v-if="item.type==='Brain Injury'" v-model=item.braininjurytype label="Select type" :items="brain_injury_types"></v-select>
											<v-slider v-if="item.type==='Burn Wound'" v-model=item.burnsurfacearea label="Surface Area Affected" hint="Enter a fraction between 0 and 1." :max="1" :min="0" :step="0.1">
												<template v-slot:append>
													<v-text-field v-model="item.burnsurfacearea" hide-details single-line density="compact" style="width: 90px"></v-text-field>
												</template>
											</v-slider>
											<v-slider v-if="(item.type!='Hemorrhage') && (item.type!='Burn Wound') && (item.type!='Cardiac Arrest')" v-model="item.severity" label="Severity" class="align-center" hint="Enter a fraction between 0 and 1." :max="sev_max" :min="sev_min" :step="0.1">
												<template v-slot:append>
													<v-text-field v-model="item.severity" hide-details single-line density="compact" style="width: 90px"></v-text-field>
												</template>
											</v-slider>
											<v-btn class='text-right' @click="changeAction(item.region)">Remove Injury</v-btn>
											<br>
											<br>
										</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			changeAction(id) {
				if (this.colors[id] == 'red') {
					this.colors[id] = 'black';
					console.log(this.colors[id])
					for (let i = 0; i < this.action.length; i++) {
						if (this.action[i].region === id) {
							this.action.splice(i, 1);
							i--; // Decrement the counter to account for the removed item
						}
					}
					this.changeColors();
				} else {
			...
			pathClicked(id) {
				...
				if (this.action.some(x => Object.keys(x).includes(id))) {
					this.action.splice(-1, 1)
				} else {
					this.action.push({
						region: id,
						type: '',
			...
			saveBiogearsFiles() {
				...
				const zip = new JSZip();
				// Add your files to the zip object
				zip.file("CustomScenario.xml", this.scenario);
				zip.file("CustomPatient.xml", this.patient);
				if (this.env) {
					zip.file("CustomEnvironment.xml", this.env);
				}
