		...
		<v-container>
			<v-card flat class="mt-n12">
				...
				<v-card-text>
					...
					<v-row>
						<v-col cols="10">
							<v-select prepend-icon="mdi-chart-line" v-if="array2D" label="Please select a variable to plot." v-model="selectPlot" id="selectButton" :items="allGroup.slice(1)"></v-select>
						</v-col>
						<v-col>
							<v-btn style="color: white" color="#3c2d70" v-if="array2D" @click="plotSelectedChart">Plot Chart</v-btn>
						</v-col>
					</v-row>
				</v-card-text>
				...
				<v-row v-if="plotChartVar" justify="center">
					<v-btn prepend-icon="mdi-download" style="color: white" color="#3c2d70" @click="downloadChart">Download Chart</v-btn>
					<br>
					<br>
					<br>
				</v-row>
	...
	export default {
		...
		methods: {
			async downloadChart() {
				...
				link.href = dataUrl;
				link.download = this.selectPlot + '.png';
